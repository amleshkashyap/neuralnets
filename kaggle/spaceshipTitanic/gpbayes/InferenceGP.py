...
class InferenceGP:
    def __init__(self, model, likelihood, criterion, bestResPath):
        self.model = model
    ...
    def predict(self, xTensor, yTensor, mode = 'test'):
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            likelihoods = self.likelihood(self.model(xTensor))

            # validation loss using the selected loss function
            # valLoss = self.criterion(likelihoods, yTensor).item()

            # get probabilities
            probabilities = likelihoods.mean

            print("Raw Probabilities (First 10):", probabilities[:10].squeeze().tolist())
